...
Protected Class UTI
	...
	#tag Method, Flags = &h0
		 Shared Function Description(uti as CFStringRef) As String
		  #if targetMacOS
		    soft declare function UTTypeCopyDescription lib CarbonFramework (uti as CFStringRef) as Ptr
	...
	#tag Method, Flags = &h0
		 Shared Function CreateFromOSType(tag as OSType, conformsTo as String = "") As String
		  #if targetMacOS
		    soft declare function UTCreateStringForOSType lib CarbonFramework (inOSType as OSType) as Ptr
		    // Caution: If this would return a CFStringRef, we'd have to Retain its value!
		    // Instead, "new CFString()" takes care of that below
	...
	#tag Method, Flags = &h21
		Private Shared Function CreateUTI(tag as CFStringRef, tagClass as Ptr, conformsTo as String = "") As String
		  #if targetMacOS
		    soft declare function UTTypeCreatePreferredIdentifierForTag lib CarbonFramework (inTagClass as Ptr, inTag as CFStringRef, inConformingToUTI as CFStringRef) as Ptr
		    // Caution: If this would return a CFStringRef, we'd have to Retain its value!
	...
	#tag Method, Flags = &h21
		Private Shared Function ResolveTagClass(symbol as String) As Ptr
		  #if targetMacOS
		    dim p as Ptr = CFBundle.CarbonFramework.DataPointer(symbol)
		    if p <> nil then
		      return p.Ptr(0)
		    else
		      return nil
