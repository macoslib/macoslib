...
Class CFType
	#tag Method, Flags = &h1
		Protected Sub Adopt(ref as Ptr, hasOwnership as Boolean)
		  // Adoption usually takes ownership, meaning that this instance will release
		  // the ref once it gets destructed.
		  // Therefore, pass true (or nothing) when getting a CFRef from a CF...Create or CF...Copy invocation.
		  // Otherwise, pass false to have a Retain being added to this object or call Retain explicitly
	...
	#tag Method, Flags = &h0
		Function TypeID() As CFTypeID
		  #if TargetMacOS
		    soft declare function CFGetTypeID lib CarbonFramework (cf as Ptr) as CFTypeID
		    
		    if me.mRef <> nil then
		      return CFGetTypeID(me.mRef)
		    end if
		  #endif
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		 Shared Function NewObject(ref as Ptr, hasOwnership as Boolean = false) As CFType
		  ...
		  dim theTypeID as UInt32 = TypeValue(CFGetTypeID(ref))
		  
		  select case theTypeID
		    
		  case TypeValue(CFArray.ClassID)
		    dim b as new CFArray(ref, hasOwnership)
		    return b
		  ...
		  case TypeValue(CFData.ClassID)
		    dim b as new CFData(ref, hasOwnership)
		    return b
		  ...
		  case TypeValue(CFDictionary.ClassID)
		    dim b as new CFDictionary(ref, hasOwnership)
		    return b
		  ...
		  case TypeValue(CFString.ClassID)
		    dim s as new CFString(ref, hasOwnership)
		    return s
	...
	#tag Method, Flags = &h0
		Sub Constructor(ref as Ptr, hasOwnership as Boolean = false)
		  // This is the preferred constructor for all CFType subclasses.
		  //
	...
	#tag Method, Flags = &h0
		 Shared Function RefCount(ref as Ptr) As Integer
		  #if TargetMacOS
		    soft declare function CFGetRetainCount lib CarbonFramework (cf as Ptr) as Integer
		    if ref <> nil then
	...
	#tag Method, Flags = &h0
		Function IsNULL() As Boolean
		  return me is nil or me.mRef is nil
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		 Shared Function TypeDescription(id as CFTypeID) As String
		  #if TargetMacOS
		    soft declare function CFCopyTypeIDDescription lib CarbonFramework (id as CFTypeID) as Ptr
		    // Caution: If this would return a CFStringRef, we'd have to Retain its value!
		    // Instead, "new CFString()" takes care of that below
		    
		    if id.opaque <> 0 then
		      return new CFString(CFCopyTypeIDDescription(id), true)
		    end if
		  #endif
		End Function
	#tag EndMethod
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Getter
			Get
			#if TargetMacOS
			soft declare function CFCopyDescription lib CarbonFramework (cf as Ptr) as Ptr
			// Caution: If this would return a CFStringRef, we'd have to Retain its value!
			// Instead, "new CFString()" takes care of that below
