...
Protected Module CoreGraphics
	...
	#tag Method, Flags = &h0
		Function CGRectInfinite() As CGRect
		  ...
		  dim p as Ptr = carbon.DataPointer("CGRectInfinite")
		  if p = nil then
		    dim oops as CGRect
		    return oops
		  end if
	...
	#tag Method, Flags = &h0
		Function CGRectNull() As CGRect
		  ...
		  dim p as Ptr = carbon.DataPointer("CGRectNull")
		  if p = nil then
		    dim oops as CGRect
		    return oops
		  end if
	...
	#tag Method, Flags = &h0
		Function CGAffineTransformIdentity() As CGAffineTransform
		  ...
		  dim p as Ptr = carbon.DataPointer("CGAffineTransformIdentity")
		  if p = nil then
		    dim oops as CGAffineTransform
		    return oops
		  end if
	...
	#tag Method, Flags = &h1
		Protected Function CArray(rects() as CGRect) As MemoryBlock
		  ...
		  for i as Integer = 0 to UBound(rects)
		    rectsArray.StringValue(offset, CGRect.Size) = rects(i).StringValue(not targetBigEndian)
	...
	#tag Structure, Name = CGPatternCallbacks, Flags = &h0
		version as Integer
		  drawPattern as Ptr
		releaseInfo as Ptr
	#tag EndStructure
