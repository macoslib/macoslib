	...
	#tag Method, Flags = &h21
		Private Shared Function MakeSubclass(className as String = NSClassName, superclassName as String = "IKImageBrowserView") As Ptr
		  //this is Objective-C 2.0 code (available in Leopard).  For 1.0, we'd need to do it differently.
		    ...
		    'dim Meth as Ptr = class_getInstanceMethod( newClassId, Cocoa.NSSelectorFromString( "newCellForRepresentedItem:" ))
		    'if Meth=nil then  BREAK
		    'dim imp as Ptr = AddressOf override_NewCell
		    '
		    'if imp=nil then BREAK
		    '
		    'call  method_setImplementation( Meth, imp )
